(function (name, context, definition) {
    ...
    Tabs.prototype._init = function() {
        ...
        for (i = this.tabs.length - 1; i > -1; i--) {
            var tab = this.tabs[i];
            var panel = this.panels[i];
            var preSelected = tab.className.match(/\btab--active/);
    ...
    Tabs.prototype.toggle = function(tab) {
        ...
        this.tabs[this.selectedIndex].className = this.tabs[this.selectedIndex].className.replace('tab--active', '');
...
var instance = new Tabs( document.querySelector('.tabs ul') );
